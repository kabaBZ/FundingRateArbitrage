...
def get_server_time():
    ...
    try:
        ...
        if time_response.get("retCode") == 0:
            return datetime.fromtimestamp(int(time_response["result"]["timeSecond"]))
...
def main(symbol, time_tuple, seperate_into=1):
    try:
        ...
        target_open_time = server_time.replace(hour=hour, minute=minute, second=second-1, microsecond=0)
        target_close_time = server_time.replace(hour=target_hour, minute=target_minute, second=target_second, microsecond=500000)
        if target_hour ==0:
            tomorrow = server_time + timedelta(days=1)
            target_close_time = tomorrow.replace(hour=target_hour, minute=target_minute, second=target_second, microsecond=100)
        ...
        current_balance = decimal(client.get_wallet_balance(
            accountType="UNIFIED",
            coin="USDT"
        )['result']['list'][0]['totalAvailableBalance']) / decimal(seperate_into)
        ...
        max_order_qty = decimal(instrument_info['lotSizeFilter']['maxMktOrderQty'])  # 最大下单数量
        min_order_qty = decimal(instrument_info['lotSizeFilter'].get('minOrderQty', qty_step))  # 最小下单数量
        ...
        qty = max(min_order_qty, min(qty, max_order_qty))  # 确保在最小和最大下单限制之间
